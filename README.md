...
- **인앱 AI (옵트인)** — 설정에서 켠 뒤에만 동작합니다. API 키는 브라우저에 두지 않고 Supabase Edge Function이 xAI를 호출합니다.
  - **자연어 거래 초안** — 거래 추가 시트에 문장으로 입력하면 금액·구분·카테고리·메모를 폼에 채웁니다. **적용은 초안만**이며, 원장 저장은 사용자가 확인한 뒤 기존 저장 버튼으로만 이뤄집니다.
  - **월 인사이트·절약 팁** — 요약 화면에서 집계·예산 달성 현황 등을 바탕으로 짧은 인사이트 문장과 규칙 기반 절약 팁을 봅니다.
  - **월 마감 점검 내러티브** — 과거 월 요약에서 빠진 고정 항목·중복 의심·메모 없는 큰 지출·예산 초과·목표 미달 등 점검을 펼쳐 볼 수 있고, 그 결과를 짧은 글로 요약합니다.
- **AI 브라우저 에이전트 연동 (WebMCP)** — 브라우저 AI 에이전트(Chrome의 Gemini 등, WebMCP 지원 브라우저)가 앱에 등록된 도구를 호출합니다. 요약 화면에서는 예산 페이스, 통계 화면에서는 월별 추이·카테고리 지출·전월 대비, 화면과 무관하게 지난달 마감 점검을 물을 수 있습니다. 인앱 AI와 별개이며, **앱 안 챗봇 UI는 없습니다.**
...
| **내역**   | 월·구분·카테고리·메모로 필터링한 거래 목록. 하단 **+** 또는 화면에서 추가·수정하고, **보내기**로 필터된 내역을 엑셀 파일로 보냅니다. 인앱 AI 켠 편집터는 추가 시 자연어 초안을 쓸 수 있습니다. |
| **통계**   | 기간(3·6·12개월)을 정해 월별 수입·지출·저축·투자·수지 추이와 지출 카테고리 비교를 봅니다.                                                                                                      |
| **설정**   | 카테고리(아이콘·예산·목표·예산 페이스), 고정 항목, 가계부·통화 기준, **인앱 AI 고지·사용 토글**, 로그아웃.                                                                                     |
...
3. **요약**에서 이번 달 수지와 예산 초과 여부, 남은 일수 기준 하루 허용액을 확인합니다. AI가 켜져 있으면 인사이트·절약 팁, 과거 월이면 마감 점검도 봅니다.
...
- 인앱 AI 채팅, 통계 기간 AI 해설, 잔여 쿼터 UI 등 후속 인앱 AI 기능

거래 내역은 기간 제한 없이 보존되며, 통화는 **KRW(원)** 기준·**달력 월(1일~말일)** 집계입니다.  
인앱 AI는 기본 꺼져 있으며(dark launch), 서버 킬 스위치(`AI_FEATURES_ENABLED`)와 사용자 옵트인이 모두 켜져야 호출됩니다.
...
자세한 제어 흐름·에러 코드는 `docs/4. ai-provider-paid-feature-candidates.md`와 `docs/4-1. ai-provider-paid-dev-tracker.md`를 참고하세요.
