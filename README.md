...
| **월요약**    | 연·월을 선택해 총수입·총지출·총저축·총투자·수지, 예산/목표 달성 현황(예산 페이스 포함), 차트를 봅니다.                        |
| **거래 내역** | 월·구분·카테고리·메모로 필터링한 거래 목록. 하단 + 버튼으로 빠르게 추가하고, **보내기**로 필터된 내역을 엑셀 파일로 보냅니다. |
| **통계**      | 기간을 정해 월별 수입·지출·저축·투자·수지 추이와 지출 카테고리 비교를 봅니다.                                                 |
| **설정**      | 카테고리(아이콘·예산·목표·예산 페이스), 고정 항목을 관리하고 가계부·통화 기준을 확인한 뒤 로그아웃합니다.                     |

월요약은 **확인 전용**입니다. 거래를 넣거나 고치려면 거래 내역이나 설정 화면을 사용합니다.

## 사용 흐름 예시

1. 가입 후 **설정**에서 카테고리 아이콘·예산·저축 목표·예산 페이스와 고정 항목(월급, 월세 등)을 정합니다.
2. 평소에는 하단 **+** 버튼이나 **거래 내역**에서 지출·수입을 기록합니다.
3. **월요약**에서 이번 달 수지와 예산 초과 여부, 남은 일수 기준 하루 허용액을 확인합니다.
4. **통계**에서 몇 달간의 패턴을 보며 다음 달 계획에 참고합니다.
5. 필요하면 **거래 내역**의 **보내기**로 해당 월 내역을 엑셀로 받아 백업하거나 다른 도구에서 활용합니다.
