...
- **거래 내역 관리** — 월·구분·카테고리·메모로 내역을 찾고, 추가·수정·삭제할 수 있습니다. 메모를 입력하면 과거 기록과 카테고리 이름을 바탕으로 카테고리를 추천하고, 같은 금액·구분·카테고리의 유사 거래가 있으면 저장 전에 확인을 요청합니다.
...
- **엑셀 내보내기** — 거래 내역 화면에서 현재 필터(월·구분·카테고리·메모)에 맞는 내역을 `.xlsx` 파일로 받을 수 있습니다.
- **통계 보기** — 최근 3·6·12개월 추이와 카테고리별 지출을 차트로 비교합니다.
- **설정 추천** — 최근 완료된 6개월의 거래를 앱 안에서 계산해 지출 예산 조정안과 반복 고정 항목 후보를 보여 줍니다. 추천은 폼을 채우는 참고용 초안이며, 사용자가 저장하기 전에는 설정을 바꾸지 않습니다.
- **인앱 AI (옵트인)** — 설정에서 고지를 확인하고 켠 뒤에만 동작합니다. API 키는 브라우저에 두지 않고 Supabase Edge Function이 OpenAI를 호출합니다.
  ...
  - **월 인사이트** — 원시 거래가 아닌 월 합계·예산/목표 달성률·계획 대비 실제 지출 비중·상위 지출 같은 집계만 전송해 짧은 조언을 만듭니다.
  - **기간 해설** — 통계 화면의 3·6·12개월 합계와 상위 지출 카테고리·최근 증감을 집계해 흐름 해석과 다음 행동을 제안합니다.
  - **월 마감 점검 내러티브** — 과거 월 요약에서 빠진 고정 항목·중복 의심·메모 없는 큰 지출·예산 초과·목표 미달 등 점검을 펼쳐 볼 수 있고, 그 결과를 짧은 글로 요약합니다.
- **AI 브라우저 에이전트 연동 (WebMCP)** — WebMCP 지원 브라우저의 AI 에이전트가 앱에 등록된 6개 도구를 호출합니다. 요약 화면에서는 전체·카테고리별 예산 페이스, 통계 화면에서는 월별 추이·카테고리 상세·최근 두 달 비교, 로그인 후에는 지정한 달의 마감 점검을 물을 수 있습니다. 원장을 편집하는 도구는 없으며, `month_close_review`만 점검 전에 해당 월의 고정 항목을 실제 거래로 멱등 반영합니다. 인앱 AI와 별개이며, **앱 안 챗봇 UI는 없습니다.**
...
| **요약**   | 연·월을 선택해 총수입·총지출·총저축·총투자·수지, 예산/목표 달성 현황(예산 페이스 포함), 차트를 봅니다. 인앱 AI가 켜져 있으면 월 인사이트, 과거 월에는 마감 점검도 표시됩니다.                                                           |
| **내역**   | 월·구분·카테고리·메모로 필터링한 거래 목록. 하단 **+** 또는 화면에서 추가·수정하고, **엑셀**로 필터된 내역을 내보냅니다. 메모 기반 카테고리 추천과 중복 의심 안내를 제공하며, 인앱 AI를 켜면 거래 추가 시 자연어 초안을 쓸 수 있습니다. |
| **통계**   | 기간(3·6·12개월)을 정해 월별 수입·지출·저축·투자·수지 추이와 지출 카테고리 비교를 봅니다. 인앱 AI가 켜져 있으면 같은 집계로 기간 흐름과 다음 행동을 해설합니다.                                                                         |
| **설정**   | 카테고리(아이콘·예산·목표·예산 페이스)와 고정 항목을 관리하고, 가계부 이름·통화 기준을 확인합니다. 최근 완료된 6개월을 바탕으로 예산과 고정 항목 초안을 제안하며, **인앱 AI 고지·사용 토글**과 로그아웃도 이 화면에 있습니다.           |

요약 화면은 거래 CRUD 기준 **확인 전용**입니다. 거래를 넣거나 고치려면 내역 화면이나 하단 **+**, 설정(고정 항목·카테고리)을 사용합니다.

## 사용 흐름 예시

1. 가입 후 **설정**에서 카테고리 아이콘·예산·저축 목표·예산 페이스와 고정 항목(월급, 월세 등)을 정합니다. 거래 이력이 쌓이면 예산 제안과 반복 항목 초안을 확인해 필요한 것만 저장합니다. 인앱 AI를 쓰려면 고지를 확인한 뒤 **인앱 AI 사용**을 켭니다.
2. 평소에는 하단 **+** 버튼이나 **내역**에서 지출·수입을 기록합니다. 메모에 맞는 카테고리 추천을 고르고, 중복 의심 안내가 나오면 기존 거래를 확인합니다. AI가 켜져 있으면 자연어 한 줄로 초안을 채운 뒤 확인하고 저장합니다.
3. **요약**에서 이번 달 수지와 예산 초과 여부, 남은 일수 기준 하루 허용액을 확인합니다. AI가 켜져 있으면 집계 기반 인사이트를 보고, 과거 월에서는 마감 점검도 봅니다.
4. **통계**에서 몇 달간의 패턴을 확인하고, AI가 켜져 있으면 같은 집계에 대한 기간 해설을 다음 달 계획에 참고합니다.
5. 필요하면 **내역**의 **엑셀**로 해당 월 내역을 받아 백업하거나 다른 도구에서 활용합니다.
...
| `src/domain/`           | 월 요약, 예산 페이스, 통계, 추천·중복 탐지, 마감 점검, 입력 검증처럼 I/O 없는 순수 계산  |
| `src/ai/`               | 인앱 AI 클라이언트 계약, 집계 페이로드 빌더, 캐시 해시와 사용자 설정 게이트              |
...
flowchart TB
    subgraph BROWSER["브라우저 (React SPA) — JWT만 소지"]
        ...
        L21["21. 표시 정제<br/>기능별 렌더링"]
        L22["22. 표시 또는 초안 반영<br/>원장 저장은 사용자 확인 후"]
        L1 --> L2 --> L3 --> L4 --> L5
        L20 --> L21 --> L22
    end
    ...
    L0["0. 부팅 시 설정 파싱<br/>OPENAI_MODEL · OPENAI_REASONING_EFFORT<br/>폴백 없음 → 502"] --> L6
```
...
flowchart TD
    ...
    F -->|아니오| E3["flag_off · 403<br/>(읽기 AI 카드는 조용히 숨김)"]
    F -->|예| G{"옵트인 + 고지 버전 일치?"}
...
클라이언트는 이 7개 코드를 `AiClientError`로 받아 각각 한국어 메시지를 붙입니다. 대시보드·통계의 AI 카드와 마감 내러티브는 `flag_off`를 **숨김**으로 처리하므로, 킬 스위치를 내려도 도메인 산출물인 마감 점검 항목은 그대로 남습니다. 이미 열린 자연어 거래 초안에서 호출이 거부되면 해당 입력 블록에는 오류 메시지를 표시합니다.
...
flowchart TB
    subgraph HASHIN["해시 입력 (클라이언트)"]
        I1["캐시 기능별 입력<br/>월 인사이트 · 기간 해설 · 마감 내러티브"]
...
한도 자체는 **migration 0018로 비구속**입니다. 이 배포는 사용자가 곧 OpenAI 계정 소유자이므로, 비용 상한은 앱의 토큰 회계가 아니라 프로바이더의 지출 한도에 두는 것이 맞습니다(그 상한은 우리 회계가 틀려도 유효합니다). 다만 `claim → settle/refund` 경로는 그대로 살아 있어 `ai_usage_daily`의 기능별 사용량 귀속은 유지됩니다. 재활성화는 한도 함수의 값을 바꾸는 마이그레이션 한 번이며, 아래 타임아웃 회계 갭을 먼저 보정해야 합니다.
...
flowchart LR
    subgraph W["WebMCP — 브라우저 에이전트"]
        ...
        W3["기존 도메인·데이터 계층<br/>조회 중심 · 원장 편집 도구 없음"]
        W1 --> W2 --> W3
    end
...
| 쓰기 권한      | 원장 편집 도구 없음¹          | 없음(초안만, 저장은 사용자 확인 경로) |
| 앱 내 챗봇 UI  | 없음(의도된 설계)             | 없음(의도된 설계)                     |

¹ `month_close_review`는 정확한 점검을 위해 조회 전에 해당 월의 고정 항목을 실제 거래로 멱등 반영합니다.
...
쿼터 예약·정산의 날짜 경계는 별도 환경 변수 없이 DB RPC가 `Asia/Seoul` 기준으로 고정합니다.
