...
function monthInsightPrompt(input: unknown): FeaturePrompt {
  return {
    ...
    system: [
      ...
      '제공된 월간 집계(summary, achievements, pace, topExpenses)가 근거입니다.',
      ...
      '',
      '# 입력 계약',
      '- month: 대상 월(YYYY-MM). groundedMonth에 문자 단위로 그대로 복사.',
      '- summary: totalIncome/totalExpense/totalSaving/totalInvestment/balance (KRW 정수).',
      '  balance = income − expense − saving − investment (흑자 양수, 적자 음수).',
      '- achievements[]: type=expense|saving, status, target, actual.',
      '  expense: 초과|주의|정상. saving: 달성|근접|진행중.',
      '  초과분 ≈ max(0, actual−target), 잔여 ≈ max(0, target−actual).',
      '- pace 속성이 있으면(빈 배열 포함) 현재 월, 없으면 마감된 월.',
      '  pace 항목: remainingBudget, daysRemaining, dailyAllowance, status.',
      '- topExpenses[]: name, amount, pct(총지출 대비).',
      ...
      '- 소득 대비 지출·저축 비율 (income>0일 때).',
      '- 적자(−balance)와 최대 초과분의 관계: 한 항목 조정이 수지에 얼마나 기여할지.',
      '- 흑자와 저축 잔여: min(흑자, 목표 잔여)로 배정 여력.',
      '- 여러 초과/주의가 있으면 효과가 큰 순서로 순위 (금액·잔여 한도 기준).',
      '- 현재 월: daysRemaining × dailyAllowance·remainingBudget으로 남은 기간 운영 한도.',
      '- topExpenses 집중과 예산 상태를 묶어, 한 축 재설계 vs 여러 항목 동시 관리 중 선택.',
      '- 현재 월에서 이미 초과한 항목은 과거 제약으로 언급 가능. 앞으로 조절할 1순위는',
      '  아직 remainingBudget이 남은(비초과) 항목에 두는 편이 실용적입니다.',
      '',
      '# 내부 작업 순서 (출력 금지)',
      '1. 현재 월 / 마감 월 판별.',
      ...
      '',
      '# 마감된 월 (pace 속성 없음) — 3필수, 여유 시 4',
      '관점: 이번 달 실적을 근거로 다음 달을 어떻게 설계할지.',
      '1. 핵심 조정: 가장 중요한 예산 신호(최대 초과, 또는 초과 없을 때 top 지출)를 근거로',
      ...
      '',
      '# 현재 월 (pace 속성 있음) — 3필수, 여유 시 4',
      '관점: 현재 실적·페이스를 근거로 남은 기간을 어떻게 운영할지.',
      '1. 운영 강도: daysRemaining, 수지, 저축 상태를 근거로 조임·유지·저축 보호 중 기조.',
      '2. 조절 1순위: 비초과 pace(또는 대체 근거)에서 위험도 높은 항목 하나 + remainingBudget/dailyAllowance 한도.',
      '3. 목표 보호 순서: 남은 변동 지출과 저축·수지 중 무엇을 먼저 지킬지.',
      '4. 선택: 남은 기간 중간 점검 시점과 그때 볼 지표 하나.',
      '',
      '# 형식',
      '- 불릿 3~4개, 각 1~2문장, 존댓말. 번호·이모지·마크다운 기호 금지.',
      '- 금액은 ₩와 천 단위 구분(예: ₩1,200,000). 반올림·추정에는 "약".',
      '- 비난·공포 조장·근거 없는 격려("잘하고 계세요", "화이팅")·공허한 "절약하세요" 금지.',
      '- 같은 조언·같은 숫자를 불릿 간에 반복하지 마세요.',
      '- groundedMonth는 입력 month와 동일.',
      '',
      ...
      '좋음(정보+α): "여가 잔여 예산 ₩…·하루 허용 ₩…이 있으므로, 저축 목표 잔여를 먼저 확보한 뒤',
    ...
    schema: {
      ...
      properties: {
        bullets: {
          type: 'array',
          description:
            '지금 정보(근거 숫자·상태) + 조언α(우선순위·트레이드오프·규모·순서·설계). 둘 다 포함.',
          minItems: 3,
          maxItems: 4,
          items: {
            type: 'string',
